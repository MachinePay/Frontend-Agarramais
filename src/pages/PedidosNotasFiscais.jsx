...
export function PedidosNotasFiscais() {
  ...
  const sincronizarNFeMail = async (numeroPedido) => {
    try {
      ...
      const { atualizados = 0, criados = 0, notasEncontradas = 0 } = response.data || {};
      setSuccess(
        `Sincronização concluída: ${notasEncontradas} nota(s) encontrada(s) na NFeMail, ${atualizados} atualizada(s), ${criados} criada(s).`,
      );
      await carregarRegistros();
    } catch (err) {
          ...
          <div className="card">
            ...
            <div className="border-t border-gray-200 pt-4">
              <h3 className="text-sm font-semibold text-gray-800 mb-2">
                Sincronizar com NFeMail
              </h3>
              <p className="text-xs text-gray-500 mb-3">
                Busca notas fiscais recebidas na NFeMail e preenche
                automaticamente número da nota, data, CIF/FOB, transportadora
                e valor dos pedidos já cadastrados (não altera coleta nem
                cotação).
              </p>
              <button
                type="button"
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            type="button"
                            className="px-3 py-1 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 disabled:opacity-60"
                            onClick={() => sincronizarNFeMail(item.numeroPedido)}
                            disabled={sincronizando}
                          >
                            NFeMail
                          </button>
